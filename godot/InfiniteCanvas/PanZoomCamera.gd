	...
	if _is_input_enabled:
		...
		if event is InputEventScreenDrag:
			...
			if _events.size() == 1:
				...
				if (_events[0].position + _events[0].relative).distance_to(_events[1].position + _events[1].relative) > _events[0].position.distance_to(_events[1].position):

...
func _ready():
	...
	_export_dialog.current_dir = Settings.get_value(
		Settings.GENERAL_DEFAULT_PROJECT_DIR, docs_folder
	)
	...
	_unsaved_changes_dialog.connect(
		"discard_changes", self, "_on_close_file_with_changes_discarded"
	)
...
func _is_mouse_on_ui() -> bool:
	...
	return (
		...
		|| Utils.is_mouse_in_control(_toolbar)
		|| Utils.is_mouse_in_control(_statusbar)
		|| Utils.is_mouse_in_control(_file_dialog)
		|| Utils.is_mouse_in_control(_about_dialog)
		|| Utils.is_mouse_in_control(_settings_dialog)
	)
...
func _is_dialog_open() -> bool:
	return (
		...
		|| _about_dialog.visible
		|| _settings_dialog.visible
		|| _generic_alert_dialog.visible
	)

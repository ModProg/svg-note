...
# TODO make this unhandled again
func _input(event):
	# Mouse to gesture.
	if event is InputEventMouseButton:
		...
		if last_mouse_press:
			if last_mouse_press.button_index == BUTTON_MIDDLE:
				...
				emit(
					"twist",
					InputEventScreenTwist.new(
						{
							"position": last_mouse_press.position,
